...
impl VirtualMemoryManager {
    ...
    pub fn allocate_range(&mut self, size: usize) -> Option<PageRange> {
        let index = self.free_ranges.iter().position(|range| range.len() >= size as u64)?;
        let mut range = self.free_ranges[index];
        let prev = range.start;
    ...
    pub fn free(&mut self, range: PageRange) {
        ...
        if let Some(index_prev) = self.free_ranges.iter().position(|prev_range| range.start >= prev_range.start) {
            let prev_range = self.free_ranges.get_mut(index_prev).unwrap();
            assert!(range.start >= prev_range.end, "Trying to free a page range that was already (partially) free!");
            ...
            if let Some(next_range) = self.free_ranges.get(index + 1) && range.end == next_range.start {
