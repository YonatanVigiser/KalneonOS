...
enable_paging:
  ...
  ; Enable PAE (Physical Address Extension) - PSE not needed for PAE paging
  mov eax, cr4
...
long_mode_start:
  ...
  ; Reload segment registers (use null selector for data segments in 64-bit mode)
  xor ax, ax
  mov ds, ax
  mov es, ax
  mov fs, ax
  ...
  ; Set up properly aligned stack for x86_64
  mov rsp, stack_top

  ; Enable SSE (required for x86_64 calling convention)
  mov rax, cr0
  and rax, 0xFFFB    ; Clear CR0.EM (bit 2) - disable emulation
  or rax, 0x2        ; Set CR0.MP (bit 1) - monitor coprocessor
  mov cr0, rax
...
gdt64:
  ...
  dq 0x00209A0000000000  ; 64-bit code segment (L=1, D=0, P=1, DPL=0, S=1, Type=0xA)
  dq 0x0000920000000000  ; 64-bit data segment (ignored in 64-bit mode but present)
gdt64_end:

gdt64_desc:
  dw gdt64_end - gdt64 - 1  ; Limit (2 bytes)
  dd gdt64                   ; Base (4 bytes, 32-bit address)

; Far jump pointer for transitioning to 64-bit mode
far_jump_ptr:
  dd long_mode_start  ; Offset (4 bytes)
  dw 0x08             ; Selector (2 bytes)
...
align 16
stack:
  resb 0x100000    ; 1MB stack (increased from 64KB)
stack_top:
